...
async function getHealth(runtime: RuntimeAdapter): Promise<RuntimeHealth> {
  ...
  // biome-ignore lint: cache is guaranteed populated immediately above
...
IMPORTANT: Use exact file paths from the source headers. Do NOT abbreviate paths.`;
    ...
    const testableRuntimes = runtimes.filter(
      (r) =>
        r.supportsCustomPrompt && ["claude", "codex", "gemini", "kimi", "copilot"].includes(r.name),
    );
    ...
    for (const runtime of testableRuntimes) {
      it(`${runtime.name}: finds issues in obviously buggy code`, async () => {
        ...
        try {
          ...
          try {
            // biome-ignore lint: the preceding expect() guarantees jsonMatch is non-null
          ...
          expect(
            issueText.includes("sql") ||
              issueText.includes("injection") ||
              issueText.includes("concatenat"),
          ).toBe(true);

          console.log(
            `  [pass] ${runtime.name}: found ${parsed.issues.length} issue(s) in ${(result.durationMs / 1000).toFixed(1)}s`,
          );
        } finally {
    ...
    for (const runtime of runtimes) {
      ...
      it(`${runtime.name}: execute trivial prompt (if available)`, async () => {
        ...
        try {
          ...
          console.log(
            `  [pass] ${runtime.name}: ${result.raw.length} chars in ${(result.durationMs / 1000).toFixed(1)}s (exit ${result.exitCode})`,
          );
        } finally {
